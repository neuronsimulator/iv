...
void GraphicBlock::Normalize (Perspective& np) {
	...
	np.x0 = iv26_round(hfactor * float(np.x0));
	np.width = p->width;
	np.curx = iv26_round(hfactor * float(np.curx));
	np.curwidth = iv26_round(hfactor * float(np.curwidth));
	np.sx = iv26_round(hfactor * float(np.sx));
    }
    if (p->height != np.height) {
	vfactor = float(p->height) / float(np.height);
	np.y0 = iv26_round(vfactor * float(np.y0));
	np.height = p->height;
	np.cury = iv26_round(vfactor * float(np.cury));
	np.curheight = iv26_round(vfactor * float(np.curheight));
	np.sy = iv26_round(vfactor * float(np.sy));
    }
}
...
void GraphicBlock::Zoom (Perspective& np) {
	...
	_x0 = iv26_round((_x0 + dx - halfw)*factor + halfw);
	_y0 = iv26_round((_y0 + dy - halfh)*factor + halfh);
	p->width = iv26_round(p->width * factor);
	p->height = iv26_round(p->height * factor);
	p->curx = iv26_round(float(cx) * factor) - halfw;
	p->cury = iv26_round(float(cy) * factor) - halfh;
    }
    _mag *= factor;
}
...
void GraphicBlock::SetMagnification (float m) {
	...
	_x0 = iv26_round((_x0 - halfw)*factor + halfw);
	_y0 = iv26_round((_y0 - halfh)*factor + halfh);
	p->width = iv26_round(p->width * factor);
	p->height = iv26_round(p->height * factor);
	p->curx = iv26_round(float(cx) * factor) - halfw;
	p->cury = iv26_round(float(cy) * factor) - halfh;
...
void GraphicBlock::RateScroll (Event& e) {
    ...
    do {
        ...
        if (dx == 0 && dy == 0) {
            ...
            double angle = atan2((double)dy, (double)dx)*180/M_PI;
