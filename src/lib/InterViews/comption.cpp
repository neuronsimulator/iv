...
static GlyphIndex next_forced_break(
    ...
    return Math::min((long)i, long(count - 1));
}
...
static GlyphIndex fill_arrays (
    ...
    return Math::min((long)last, long(count - 1));
}
...
GlyphIndex Composition::beginning_of(CompositorIndex item) const {
    CompositorIndex i = Math::max(0L, Math::min(long(item/2), long(breaks_->count()-1)));
...
CompositorIndex Composition::item(GlyphIndex index) const {
    ...
    c->item_ = Math::min(Math::max(0L, item_), long(count-1));
...
void Composition::pick(
	...
	GlyphIndex b_count = breaks_->count();
		...
		index = Math::max(br.first_, Math::min(long(br.last_ + 1), index));
	    } else {
		/*
